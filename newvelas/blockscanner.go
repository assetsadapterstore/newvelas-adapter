...
func (bs *BlockScanner) newBlockNotify(block *quorum.EthBlock, isFork bool) {
...
func (bs *BlockScanner) BatchExtractTransaction(height uint64, txs []*quorum.BlockTransaction) error {
	...
	//提取工作
	extractWork := func(mTxs []*quorum.BlockTransaction, eProducer chan ExtractResult) {
		for _, tx := range mTxs {
			bs.extractingCH <- struct{}{}
			//shouldDone++
			go func(mTx *quorum.BlockTransaction, end chan struct{}, mProducer chan<- ExtractResult) {
...
func (bs *BlockScanner) extractBaseTransaction(tx *quorum.BlockTransaction, result *ExtractResult) {
...
func (bs *BlockScanner) extractETHTransaction(tx *quorum.BlockTransaction, isTokenTransfer bool) map[string]*openwallet.TxExtractData {
...
func (bs *BlockScanner) extractERC20Transaction(tx *quorum.BlockTransaction, contractAddress string, tokenEvent []*quorum.TransferEvent) map[string]*openwallet.TxExtractData {
	...
	bc := bind.NewBoundContract(ethcom.HexToAddress(contractAddress), quorum.ERC20_ABI, bs.wm.RawClient, bs.wm.RawClient, nil)
...
//extractERC20Detail
func (bs *BlockScanner) extractERC20Detail(tx *quorum.BlockTransaction, contractAddress string, tokenEvent []*quorum.TransferEvent, isInput bool, extractData map[string]*openwallet.TxExtractData) []string {
...
func (bs *BlockScanner) extractSmartContractTransaction(tx *quorum.BlockTransaction, result *ExtractResult) {
